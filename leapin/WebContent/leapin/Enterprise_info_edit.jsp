...
<title>Enterprise info edit page</title>
		...
		<div class="pagecenter">
			<div class="mypage-main">
				...
				<div class="top_button_group">
					...
					<div>
						<button class="btn btn-primary" type="button" id="btn_enterprise_info_edit" name="btn_enterprise_info_edit" onClick="location.href='Controller?command=Enterprise_info_edit';">기업정보수정</button>
					</div>
					<div>
				...
				<h3>기업정보수정</h3><br/>
					<table class="table table-hover">
						...
						<tr class="form-group">
							<td class="col1"><label for="edit_c_number">사업자번호</label></td>
							<td colspan="2"><input class="form-control" type="text" id="edit_c_number" name="edit_c_number" placeholder="사업자번호 10자리를 입력해주세요" value="<%=dao.receive_c_number(loginId)%>"/></td>
						</tr>
						<tr class="form-group">
							<td class="col1"><label for="edit_enterprise_name">상호명</label></td>
							<td colspan="2"><input class="form-control" type="text" id="edit_enterprise_name" name="edit_enterprise_name" placeholder="상호를 입력해주세요" value="<%=dao.receive_enterprise_name(loginId)%>"/></td>
						</tr>
						<tr class="form-group">
							<td class="col1"><label for="edit_email">이메일</label></td>
							<td colspan="2"><input class="form-control" type="email" id="edit_email" name="edit_email" placeholder="이메일 작성을 해주세요" value="<%=dao.receive_email(loginId)%>"/></td>
						</tr>
						<tr class="form-group">
							<td class="col1">회사번호</td>
							<td colspan="2"><input class="form-control" type="number" id="edit_tel" name="edit_tel" value="<%=dao.receive_tel(loginId)%>"/></td>
						</tr>
						<tr class="form-group">
						...
						<tr class="form-group">
							<td class="col1"><label for="edit_service_area">서비스지역</label></td>
							<td colspan="2"><input class="form-control" type="text" id="edit_service_area" name="edit_service_area" placeholder="서비스지역을 입력해주세요"/></td>
						</tr>
						<tr class="form-group">
							<td class="col1"><label for="edit_content">기업소개</label></td>
							<td colspan="2"><textarea cols="50" rows="2" class="form-control" id="edit_content" name="edit_content" placeholder="기업소개를 입력해주세요"></textarea></td>
						</tr>
						<tr class="form-group">
							<td class="col1"><label for="edit_service_content">서비스소개</label></td>
							<td colspan="2"><textarea cols="50" rows="5" class="form-control" id="edit_service_content" name="edit_service_content" placeholder="서비스소개를 입력해주세요"></textarea></td>
						</tr>
						<tr class="form-group">
							<td class="col1"><label for="edit_file">파일첨부</label></td>
							<td colspan="2">
						...
						<tr class="form-group">
							<td class="col1"><label for="edit_category">카테고리</label></td>
							<td colspan="2">
								<label><input class="form-control" type="checkbox" id="edit_category1" name="edit_category1" value="col1"/>웹개발</label>
								<label><input class="form-control" type="checkbox" id="edit_category2" name="edit_category2" value="col2"/>정보보안</label>
								<label><input class="form-control" type="checkbox" id="edit_category3" name="edit_category3" value="col3"/>어플개발</label>
							</td>
						</tr>
						<tr class="form-group">
							<td colspan="3"></td>
						...
						<tr class="form-group">
							...
							<input id="btn_reset" class="btn btn-primary" type="reset"  value="다시작성"/>
							<input id="btn_enterprise_info_edit" class="btn btn-primary" type="submit"  value="수정완료"/></td>
						</tr>
						</tbody>
					</table>
				</div>
...
$(function(){
	...
	$("#edit_tel").on("input",function(){
       ...
       if(result != null){
          $(".tel.regex").html("");  
           ...
           $(".tel.regex").html("올바른 번호가 아닙니다");
       }
   })
	
	$("#btn_enterprise_info_edit").click(function(){
		var c_number = $("#edit_c_number").val();
		var e_name = $("#edit_enterprise_name").val();
		var ceo_name = $("#edit_ceo_name").val();
		...
		var service_area = $("#edit_service_area").val();
		var content = $("#edit_content").val();
		var service_content = $("#edit_service_content").val();
		var file = $("#file").val();
		console.log(file);
	...
	var command = "Enterprise_info_edit";
	console.log("command : " + command);
	$.ajax({
		type:"post",
		url:"./Controller",
		data:{"eid":loginId, "c_number" : c_number, "e_name": e_name, "ceo_name" : ceo_name, "eaddress": address, "e_tel" : tel, "service_area" : service_area, "content" : content, "service_content" : service_content, "file_name" : file_real_name, "command":"Enterprise_info_edit"},
		datatype:"json",
...
function upload(){
    ...
    $("#btn_enterprise_info_edit").prop("disabled", true);   
    ...
    $.ajax({             
        ...
        success: function (data) { 
        	...
        	$("#btn_enterprise_info_edit").prop("disabled", false);  
        },          
        error: function (e) {  
        	console.log("ERROR : ", e);     
            $("#btn_enterprise_info_edit").prop("disabled", false);    
